...
const MEMBER_CHANGE_INTERVAL = 3000;
...
const useCarouselAnimation = (topContainersRef: React.RefObject<HTMLDivElement | null>, isMobile: boolean) => {
  useEffect(() => {
    if (!isMobile) return;
    ...
    const animate = () => {
      ...
      topScrollAmount += SCROLL_SPEED;
      
      if (topScrollAmount >= maxTopScroll) {
        topScrollAmount = 0;
      }
  ...
  }, [isMobile, topContainersRef]);
};

// Custom hook for member card cycling
const useMemberCycling = (isMobile: boolean, totalMembers: number) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentMemberIndex(prev => (prev + 1) % totalMembers);
    }, MEMBER_CHANGE_INTERVAL);

    return () => clearInterval(interval);
  }, [isMobile, totalMembers]);
...
export default function About() {
  ...
  const currentMemberIndex = useMemberCycling(isMobile, MEMBER_DATA.length);
  ...
  const getCardClass = useCallback((index: number) => {
    if (!isMobile) return '';
    
    if (index === currentMemberIndex) return 'active';
    if (index === (currentMemberIndex + 1) % MEMBER_DATA.length) return 'next';
    return 'prev';
  }, [isMobile, currentMemberIndex]);
        ...
        <div className={`mobile-nav-blur${menuOpen ? ' open' : ''}`}>
          <div className="mobile-nav-content">
            ...
            <ul className="mobile-nav-links">
              <li><Link href="/" onClick={handleMenuClose}>Home</Link></li>
              <li><Link href="/About" onClick={handleMenuClose}>About</Link></li>
              <li><Link href="/ServicesPage" onClick={handleMenuClose}>Services</Link></li>
              <li><Link href="/Contact" onClick={handleMenuClose}>Contact</Link></li>
              <li>
                <button
  ...
  ), [isMobile, menuOpen, handleMenuToggle, handleMenuClose]);

  // Memoized member cards
  const MemberCards = useMemo(() => (
    <div className="memberCards mobile-carousel">
    ...
    <div className="aboutWrapper">
      ...
      <section className="aboutIntroSection">
        ...
        <div className="introWrapper">
          ...
          <div className="motto">
            <h3>THE WAY TO MOVE WITH CARE</h3>
            <p>
        ...
        <div className="footerInnerContainer">
          <div className="footerContainer">
            ...
            <div className="footerRight">
              <h3>Contact Us</h3>
              <button className="whatsappBtn" aria-label="Contact us on WhatsApp">
                <FaWhatsapp />
                Get in Touch
              </button>
            </div>
